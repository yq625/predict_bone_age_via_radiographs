...
def train_model(model, dataloader, optimizer, loss_fn, num_epochs = 10, verbose = False):
    ...
    for i in range(num_epochs):
        ...
        if (i%10 == 0):
            torch.save(best_model_wts, 'vgg16_onlyfc_40ep.wts')
...
class NET(nn.Module):
    def __init__(self , model):
        ...
        self.vgg_features = nn.Sequential(*list(model.children())[:-1])
...
best_model, loss_dict = train_model(net, dataloader, optimizer, loss_fn, num_epochs = 40)

	
torch.save(best_model.state_dict(), 'vgg16_onlyfc_40ep.pkl')

        
with open('loss_train_vgg16_onlyfc_40ep.txt', 'w') as f:
        f.write(str(loss_dict['train']))
with open('loss_validate_vgg16_onlyfc_40ep.txt', 'w') as f:
